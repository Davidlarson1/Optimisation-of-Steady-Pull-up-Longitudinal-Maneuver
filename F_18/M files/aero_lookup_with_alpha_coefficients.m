function [aero_output,CDC1]=aero_lookup_with_alpha_coefficients(alpha);
%
% m file to implement an aero table lookup for the harv data base.
%
%  format:  [aero_output]=aero_lookup_with_alpha_coefficients(alpha);
%
%  inputs:  alpha
%
%  outputs: aero_output - vector of aero lookup results (length=36)
%
%     here is the order (all derivatives per degree)
%
% (1) cy_b   ( 8) croll_b   (15) cn_b   (22) cd_0       (27) clift_0       (32) cm_0
% (2) cy_p   ( 9) croll_p   (16) cn_p   (23) cd_alpha   (28) clift_alpha   (33) cm_alpha
% (3) cy_r   (10) croll_r   (17) cn_r   (24) cd_q       (29) clift_q       (34) cm_q
% (4) cy_da  (11) croll_da  (18) cn_da  (25) cd_del     (30) clift_del     (35) cm_del
% (5) cy_del (12) croll_del (19) cn_del (26) cd_der     (31) clift_der     (36) cm_der
%                                       (37)cm_combo    (38)clift_combo    (39)cd_combo
...
%         
%    
% format:  [aero_out]=aero_lookup(alpha);
%
%
% 


if ~exist('ALPHA_BREAK')
...
%
% here are the alpha breakpoints
%
...
%
 ...
 %
 % here is cd_combo = cd0 + cd_alpha*(alpha) so seperating cd0 and cd_alpha
 %
 cd_combo = [  0.227325   0.170673    7.47685E-02    3.20977E-02    2.79305E-02 ...
	     ...
	     2.12822]';
  cd0 = interp1(ALPHA_BREAK,cd_combo,0);
  % cd_0 = ones(27,1)*cd0;
  for i = 1:length(ALPHA_BREAK)-1
      CD11(i) = (cd_combo(i+1)-cd_combo(i))/(ALPHA_BREAK(i+1)-ALPHA_BREAK(i));
  end
  CD22 = CD11(end);
  
  cd_alpha = cat(2,CD11,CD22)';
    cd_0 = cd_combo-cd_alpha.*ALPHA_BREAK;

%    cd_alpha = (cd_combo-cd_0)./(ALPHA_BREAK);
 ...
 %
 % here is cd_del (for left elevator)
 %
 ...
 %
 % here is cd_der (for right elevator)
 %
 cd_der = [ -2.63851E-03   -2.42289E-03   -2.00960E-03   -1.61668E-03   -1.11178E-03 ...
	    -4.31951E-04    4.33892E-04    1.11520E-03    1.97203E-03    2.82682E-03 ...
	     ...
	     1.25007E-03    1.13131E-03]';
 %
 % here is clift_combo = clift0 + clift_alpha*(alpha) so seperating clift0 and clift_alpha
 %
 clift_combo = [ -1.03860  -0.880907  -0.551318  -0.216206   0.154461   0.560857 ...
 ...
 clift0 = interp1(ALPHA_BREAK,clift_combo,0);
%  clift_0 = ones(27,1)*clift0;
  for i = 1:length(ALPHA_BREAK)-1
      CL11(i) = (clift_combo(i+1)-clift_combo(i))/(ALPHA_BREAK(i+1)-ALPHA_BREAK(i));
  end
  CL22 = CL11(end);
  
  clift_alpha = cat(2,CL11,CL22)'; 
  clift_0 = clift_combo-clift_alpha.*ALPHA_BREAK;
%  clift_alpha = (clift_combo-clift_0)./(ALPHA_BREAK);
 % here is clift_q
 %
 ...
 %
 % here is clift_del (for left elevator)
 %
 ...
 %
 % here is clift_der (for right elevator)
 %
 clift_der = [ 5.03735E-03    5.30820E-03    5.80124E-03    6.25003E-03    6.76123E-03 ...
	    ...
	    -2.48706E-03   -2.47676E-03]';
 %
 % here is cm_combo = cm0 + cm_alpha*(alpha) so seperating cm0 and cm_alpha
 %
 cm_combo = [9.62520E-02    8.07290E-02    5.54066E-02    2.80686E-02    5.08503E-03 ...
  ...
  cm0 = interp1(ALPHA_BREAK,cm_combo,0);
%   cm_0 = ones(27,1)*cm0;
  for i = 1:length(ALPHA_BREAK)-1
      CM11(i) = (cm_combo(i+1)-cm_combo(i))/(ALPHA_BREAK(i+1)-ALPHA_BREAK(i));
  end
  CM22 = CM11(end);
  
  cm_alpha = cat(2,CM11,CM22)';
  cm_0 = cm_combo - cm_alpha.*ALPHA_BREAK;
%   cm_alpha = (cm_combo-cm_0)./(ALPHA_BREAK);
 %
 % here is cm_q
 %
 cm_q = [   -8.68301E-02   -8.68301E-02   -8.68301E-02   -8.65683E-02   -8.53902E-02 ...
	    ...
	    -8.24668E-02]';
 %
 % here is cm_del (for left elevator)
 %
 ...
 %
% %  
% %  CDC1 = interp1(ALPHA_BREAK,cd_combo,alpha);
% %   for i = 1:length(CDC1)-1
% %       CD11(i) = (CDC1(i+1)-CDC1(i))/1;
% %   end
% %   CD22 = CD11(end);
% %   
% %   cd_alpha = cat(2,CD11,CD22)';
% %   
% %   CMC1 = interp1(ALPHA_BREAK,cm_combo,alpha);
% %   for i = 1:length(CMC1)-1
% %       CM11(i) = (CMC1(i+1)-CMC1(i))/1;
% %   end
% %   CM22 = CM11(end);
% %   
% %   cm_alpha = cat(2,CM11,CM22)';
% %   
% %   CLC1 = interp1(ALPHA_BREAK,clift_combo,alpha);
% %   for i = 1:length(CLC1)-1
% %       CL11(i) = (CLC1(i+1)-CLC1(i))/1;
% %   end
% %   CL22 = CL11(end);
% %   
% %   clift_alpha = cat(2,CL11,CL22)';
 % here is cm_der (for right elevator)
 %
...
AEROTABLENEW = [cy_b,cy_p,cy_r,cy_da,cy_del,cy_der,cy_dr, ...
    croll_b,croll_p,croll_r,croll_da,croll_del,croll_der,croll_dr, ...
    cn_b,cn_p,cn_r,cn_da,cn_del,cn_der,cn_dr, ...
    cd_0,cd_alpha,cd_q,cd_del,cd_der, ...
    clift_0,clift_alpha,clift_q,clift_del,clift_der, ...
    cm_0,cm_alpha,cm_q,cm_del,cm_der,cm_combo,clift_combo,cd_combo];
...
aero_output = interp1(ALPHA_BREAK,AEROTABLENEW,alpha);
